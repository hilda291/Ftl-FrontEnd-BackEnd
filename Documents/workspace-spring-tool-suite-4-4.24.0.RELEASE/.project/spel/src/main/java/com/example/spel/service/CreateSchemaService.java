...
public class CreateSchemaService {
    ...
    public String validateJson(String jsonFilePath, String schemaFilePath) {
        try {
            JsonNode schemaNode = mapper.readTree(new File(schemaFilePath));
            ObjectNode currencyProperty = createCurrencyProperty();
            injectReusableSchema(schemaNode, currencyProperty);
            return validateJsonAgainstSchema(jsonFilePath, schemaNode);
        } catch (IOException e) {
            return "Error reading files: " + e.getMessage();
        }
    }

    private ObjectNode createCurrencyProperty() {
    ...
    private void injectReusableSchema(JsonNode schemaNode, ObjectNode currencyProperty) {
        if (schemaNode.isObject()) {
            ...
            while (fieldNames.hasNext()) {
                ...
                if (childNode.isObject() && childNode.has("$ref") && childNode.get("$ref").asText().equals("currencyProperty")) {
                    ...
                    injectReusableSchema(childNode, currencyProperty);
                }
            }
    ...
    private String validateJsonAgainstSchema(String jsonFilePath, JsonNode schemaNode) throws IOException {
